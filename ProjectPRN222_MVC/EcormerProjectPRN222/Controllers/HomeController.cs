...
namespace EcormerProjectPRN222.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            //New products
            List<Product> top4ProductsMen = _context.Products.Where(p => p.CategoryId == 6).OrderByDescending(p => p.ProductId).Take(4).ToList();
            List<Product> top4ProductsWoman = _context.Products.Where(p => p.CategoryId == 7).OrderByDescending(p => p.ProductId).Take(4).ToList();
            List<Product> top3ProductsChildren = _context.Products.Where(p => p.CategoryId == 8).OrderByDescending(p => p.ProductId).Take(4).ToList();
